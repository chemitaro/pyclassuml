...
- E-RQ-002:
  - class field が Optional / Union / collection item として selected internal class を参照する場合、所有元 class から参照先 class への aggregation として扱う。
- E-RQ-003:
...
- E-AC-001:
  ...
  - Then: `Order` 側に黒塗り diamond を持つ composition が出る。
- E-AC-002:
  - Given: `class Order: lines: list[OrderLine]` または `coupon: Coupon | None`
  - When: `pyclassuml generate` が PlantUML を生成する
  - Then: `Order` 側に白抜き diamond を持つ aggregation が出る。
- E-AC-003:
...
- MUST:
  ...
  - PEP 604 union、`typing.Optional`、`typing.Union`、主要 collection generic の field annotation を対象にする。
- MUST NOT:
  - 対象コードを import 実行しない。
  - 対象ソースコードを書き換えない。
...
- Ask:
  - `dict[str, T]` や nested collection の扱いを MVP 以上に拡張する場合。
- Never:
