...
- MUST:
  ...
  - `AnalysisConfig.mode` は `warn | strict` に固定し、`cli_options.strict=true` なら `strict`、それ以外は config `mode` があればそれを採用し、未指定時は `warn` に写像する。
  ...
  - config read failure、invalid config value、unresolved config path、unresolved CLI root path、containment violation を hard failure とする。
- MUST NOT:
...
- AC-004:
  ...
  - When:
    - context resolve を行う。
  - Then:
    - hard failure として non-zero 相当の failure handoff を返す。config read / invalid config value / unresolved config path / unresolved CLI root path は `invalid_config_or_config_path`、containment violation は `invalid_path_or_containment` を使う。
  - 観測点:
...
- CLI 起点の `target_python` / `depth` の scalar shape は `CommandOptions` DTO の構築時 validation が守る。`config` は valid `CommandRequest` に入った CLI 値を merge/carry し、config 起点の `target_python` / `depth` は一元的に検証して、不正値は `invalid_config_or_config_path` として扱う。
...
- config read failure、invalid config value、unresolved config path、unresolved CLI root path の `failure_reason` は `invalid_config_or_config_path` に固定する。
