...
## 目的
- render と upstream 各 seam の結果を受けて、`.puml` artifact、summary、stdout/stderr routing、strict / warn exit policy を一元化する。
...
- MUST:
  ...
  - success path では summary を stdout、failure path では diagnostics / summary を stderr に流す。
  ...
  - warning diagnostics の `recoverability` に基づき、`warning_only_success` と `degraded_success` の classifier を固定し、artifact completeness を下げない operational warning は `noop`、omitted-or-ambiguous content warning は `degraded_output` を使う。
...
- AC-001:
  ...
  - Then:
    - initiative requirement の outcome table どおり、clean success / warning-only success / degraded success では `.puml` artifact が書き出され、summary は stdout に流れ、終了コードは `0` になる。
  - 観測点:
    - artifact path、stdout summary、exit code review、および success path summary counter が `DiagramModel.rendered_classes` / `rendered_relations` を起点にしていることの review。
- AC-002:
  - Actor:
    - CLI 利用者
  - Given:
...
- EC-002:
  ...
  - 期待:
    - `.puml` 成功を書かず、failure summary を stderr に流し、non-zero を返す。
  - 観測点:
