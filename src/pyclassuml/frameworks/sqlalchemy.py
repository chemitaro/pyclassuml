"""SQLAlchemy-specific relation enrichment from framework-neutral parse evidence."""
...
class SqlalchemyEnrichmentHints:
    """Seam-local additions produced by the SQLAlchemy framework support."""
...
def extract_sqlalchemy_enrichment_hints(
    parsed_modules: tuple[ParsedModule, ...],
    module_index: ModuleIndex,
    selected_classes: SelectedClasses,
    selected_relations: SelectedRelations,
) -> SqlalchemyEnrichmentHints:
    """Extract SQLAlchemy relation hints without mutating core selection."""
    ...
    for reference in sorted(
        ...
        evidence_kind = _sqlalchemy_evidence_kind(reference.reference_kind, reference.reference_owner)
    ...
    return SqlalchemyEnrichmentHints(
...
def _sqlalchemy_evidence_kind(reference_kind: str, reference_owner: str) -> str | None:
    if reference_kind == "annotation_subscript" and reference_owner == "Mapped":
        return "sqlalchemy_mapped"
    if reference_kind == "call_string_arg" and reference_owner == "relationship":
        return "sqlalchemy_relationship_string"
    return None
...
def _resolve_target(
    *,
    ...
    if len(candidates) == 0:
        return None, "sqlalchemy_relation_unresolved"
    if len(candidates) > 1:
        return None, "sqlalchemy_relation_ambiguous"

    target_class_id = candidates[0]
    if target_class_id not in selected_class_ids:
        return None, "sqlalchemy_relation_selection_outside"
...
def _warning_diagnostic(
    *,
    ...
    return Diagnostic(
        ...
        code=code,
        message=(
            "SQLAlchemy relation reference could not be converted to a selected class relation: "
